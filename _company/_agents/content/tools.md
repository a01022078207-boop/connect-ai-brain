...
_Content 에이전트가 어떤 도구를 어디까지 자율적으로 쓸 수 있는지 정의합니다._
...
### `shotlist_builder` _(예정)_
대본 → [장면·화면·자막·B-roll·길이] 샷리스트 표 자동 생성

- 아직 구현되지 않은 도구입니다. 로드맵에 있으며 향후 버전에서 추가 예정.

### `multiformat_repurpose` _(예정)_
콘텐츠 1개 → 릴스/쇼츠/카드뉴스 멀티 포맷 변주

- 아직 구현되지 않은 도구입니다. 로드맵에 있으며 향후 버전에서 추가 예정.

### `content_calendar` _(예정)_
요일·채널·포맷 기준 발행 캘린더 자동 배치
...
- 모든 외부 행동은 `_agents/content/activity.log`에 한 줄 기록 (감사용).
