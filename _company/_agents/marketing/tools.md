...
_Marketing 에이전트가 어떤 도구를 어디까지 자율적으로 쓸 수 있는지 정의합니다._
...
### `campaign_card` _(예정)_
공동구매 1건 → [채널·가설·크리에이티브·지표·합격기준] 캠페인 카드 자동 생성

- 아직 구현되지 않은 도구입니다. 로드맵에 있으며 향후 버전에서 추가 예정.

### `funnel_report` _(예정)_
노출→클릭→저장→유입→전환 퍼널 단계별 이탈 진단 리포트

- 아직 구현되지 않은 도구입니다. 로드맵에 있으며 향후 버전에서 추가 예정.

### `ab_planner` _(예정)_
후크 가설 기반 A/B 테스트 설계 + 합격 기준 산출
...
- 모든 외부 행동은 `_agents/marketing/activity.log`에 한 줄 기록 (감사용).
